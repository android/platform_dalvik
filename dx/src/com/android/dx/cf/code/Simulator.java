...
public class Simulator {
    ...
    private void checkInvokeInterfaceSupported(final int opcode, CstMethodRef callee) {
        ...
        if (dexOptions.allowAllInterfaceMethodInvokes) {
            ...
            String reason =
                    String.format(
                        "invoking a %s interface method %s.%s is experimental and " +
                        "unsupported unless --min-sdk-version >= %d (currently %d)",
            ...
            String reason =
                    String.format(
                        "invoking a %s interface method %s.%s " +
                        "requires --min-sdk-version >= %d (currently %d)",
                    invokeKind, callee.getDefiningClass().toHuman(), callee.getNat().toHuman(),
                    DexFormat.API_INVOKE_INTERFACE_METHODS, dexOptions.minSdkVersion);
            fail(reason);
        }
