    ...
    for(k = 0; k < num_compile_entries; k++) {
        ...
        if(compileTable[k].spill_loc_index >= 0) {
            if(compileTable[k].spill_loc_index > 4*(MAX_SPILL_JIT_IA-1))
                ALOGE("ERROR: spill_loc_index is wrong for entry %d: %d\n",
        ...
        if(isVirtualReg(compileTable[k].physicalType) && compileTable[k].gType == GLOBALTYPE_GG) {
            if(bb->bb_index > 0) { //non-entry block
                if(isFirstOfHandler(bb)) {
                    ...
                    ALOGI("REGALLOC: physical reg %d is used by a GG VR %d %d at beginning of BB\n", tReg, compileTable[k].regNum, compileTable[k].physicalType);
#endif
                }
            } //non-entry block
        } //if GG VR
    ...
    for(k = 0; k < num_compile_entries; k++) {
        ...
        if(indexL < 0) {
            ...
            if(num_memory_vr >= NUM_MEM_VR_ENTRY) {
                ALOGE("ERROR: exceeds size of memVRTable\n");
                dvmAbort();
            }
...
int collectInfoOfBasicBlock(Method* method, BasicBlock_O1* bb) {
    ...
    for(kk = 0; kk < bb->num_regs; kk++) {
        ...
        ALOGI("sort virtual reg %d type %d -------\n", bb->infoBasicBlock[kk].regNum,
...
#ifdef DEBUG_ALLOC_CONSTRAINT
    ALOGI("sort constraints for BB %d --------\n", bb->bb_index);
#endif
...
int codeGenBasicBlock(const Method* method, BasicBlock_O1* bb) {
    ...
    num_const_vr = 0;
#ifdef DEBUG_COMPILE_TABLE
    ALOGI("At start of basic block %d (num of VRs %d) -------\n", bb->bb_index, bb->num_regs);
    dumpCompileTable();
#endif
    ...
    for(mir = bb->jitBasicBlock->firstMIRInsn; mir; mir = mir->next) {
...
#ifdef DEBUG_COMPILE_TABLE
        ALOGI("compile table size after importing temporary info %d\n", num_compile_entries);
        ALOGI("before one bytecode %d (num of VRs %d) -------\n", bb->bb_index, bb->num_regs);
#endif
        ...
        if(isDeadStmt) { //search compileTable
            getVirtualRegInfo(infoByteCode);
#ifdef DEBUG_DSE
            ALOGI("DSE: stmt at offsetPC %d is dead\n", offsetPC);
#endif
            for(k = 0; k < num_regs_per_bytecode; k++) {
                int indexT = searchCompileTable(LowOpndRegType_virtual | infoByteCode[k].physicalType,
                                                infoByteCode[k].regNum);
        ...
        if(!isConst && !isDeadStmt)  //isDeadStmt is false when DSE_OPT is not enabled
        {
            ...
            if (retCode == 1){
                ALOGE("JIT implementation not found\n");
                return -1;
            }
            updateConstInfo(bb);
            freeShortMap();
...
#ifdef DEBUG_COMPILE_TABLE
        ALOGI("after one bytecode BB %d (num of VRs %d)\n", bb->bb_index, bb->num_regs);
#endif
    }//for each bytecode
#ifdef DEBUG_COMPILE_TABLE
    dumpCompileTable();
...
#ifdef PRINT_WARNING
            if(compileTable[k].refCount > 0)
                ALOGI("WARNING: refCount for REG %d %d is %d at end of a basic block\n", compileTable[k].regNum, compileTable[k].physicalType, compileTable[k].refCount);
#endif
            compileTable[k].refCount = 0;
...
int mergeEntry2(BasicBlock_O1* bb) {
...
#ifdef DEBUG_MERGE_ENTRY
            ALOGI("update accessType in case 2: VR %d %d accessType %d\n", regA, typeA, bb->infoBasicBlock[jj].accessType);
#endif
            hasAlias = true;
    ...
    if(!isMerged) {
...
#ifdef DEBUG_MERGE_ENTRY
        ALOGI("update accessType in case 3: VR %d %d accessType %d\n", regB, typeB, bb->infoBasicBlock[bb->num_regs].accessType);
#endif
        ...
        if(bb->num_regs >= MAX_REG_PER_BASICBLOCK) {
            ALOGE("ERROR number of VRs in a basic block\n");
            dvmAbort();
        }
        return -1;
    }
...
void updateReachingDefA(int indexToA, OverlapCase isBPartiallyOverlapA) {
    ...
    */
    ...
    if(isBPartiallyOverlapA != OVERLAP_NO) {
        ...
        if(k >= 3) {
          ALOGE("ERROR: more than 3 reaching defs\n");
        }
    ...
    for(k2 = 0; k2 < tmpInfo.num_reaching_defs; k2++ ) {
            ...
            if(currentInfo.reachingDefs[k].regNum == tmpInfo.reachingDefs[k2].regNum &&
               currentInfo.reachingDefs[k].physicalType == tmpInfo.reachingDefs[k2].physicalType) {
                ...
                if(currentInfo.reachingDefs[k].offsetPC != tmpInfo.reachingDefs[k2].offsetPC) {
                    ALOGE("ERROR: defs on the same VR %d %d with different offsetPC %d vs %d\n",
                          currentInfo.reachingDefs[k].regNum, currentInfo.reachingDefs[k].physicalType,
                ...
                if(currentInfo.reachingDefs[k].accessType != tmpInfo.reachingDefs[k2].accessType)
                    ALOGE("ERROR: defs on the same VR %d %d with different accessType\n",
                          currentInfo.reachingDefs[k].regNum, currentInfo.reachingDefs[k].physicalType);
                break;
            }
        }
        if(!merged) {
            if(currentInfo.num_reaching_defs >= 3) {
               ALOGE("ERROR: more than 3 reaching defs for currentInfo\n");
            }
...
DefUsePair* insertADef(int offsetPC, int regNum, LowOpndRegType pType, RegAccessType rType) {
    ...
    currentBB->num_defs++;
#ifdef DEBUG_REACHING_DEF
    ALOGI("insert a def at %d to defUseTable for VR %d %d\n", offsetPC,
          regNum, pType);
#endif
    return ptr;
}
...
void mergeLiveRange(int tableIndex, int rangeStart, int rangeEnd) {
...
#ifdef DEBUG_LIVE_RANGE
    ALOGI("LIVERANGE call mergeLiveRange on tableIndex %d with [%x %x]\n", tableIndex, rangeStart, rangeEnd);
#endif
...
#ifdef DEBUG_LIVE_RANGE
        ALOGI("LIVERANGE insert one live range [%x %x] to tableIndex %d\n", rangeStart, rangeEnd, tableIndex);
#endif
        return;
    }
    ...
    if(endIndex < startIndex+1) ALOGE("ERROR: mergeLiveRange endIndex %d startIndex %d\n", endIndex, startIndex);
    ///////// use ptrStart & ptrEnd_prev
    if(ptrStart == NULL || ptrEnd_prev == NULL) {
        ALOGE("ERROR: mergeLiveRange ptr is NULL\n");
        return;
    }
...
#ifdef DEBUG_LIVE_RANGE
    ALOGI("LIVERANGE merge entries for tableIndex %d from %d to %d\n", tableIndex, startIndex+1, endIndex-1);
#endif
    if(ptrStart->num_access <= 0) ALOGE("ERROR: mergeLiveRange number of access\n");
#ifdef DEBUG_LIVE_RANGE
...
void insertAccess(int tableIndex, LiveRange* startP, int rangeStart) {
    int k3, k4;
#ifdef DEBUG_LIVE_RANGE
    ALOGI("LIVERANGE insertAccess %d %x\n", tableIndex, rangeStart);
#endif
    int insertIndex = -1;
...
#ifdef DEBUG_LIVE_RANGE
    ALOGI("LIVERANGE insert %x to tableIndex %d\n", rangeStart, tableIndex);
#endif
    startP->num_access++;
    return;
}
...
#ifdef DEBUG_SPILL
        ALOGI("Skip dumpImmToMem vA %d size %d\n", vrNum, size);
#endif
        return;
    }
...
#ifdef DEBUG_SPILL
        ALOGI("Skip dumpToMem vA %d type %d\n", vrNum, type);
#endif
        return;
    }
...
#ifdef DEBUG_SPILL
            ALOGI("Skip dumpPartToMem isLow %d vA %d\n", isLow, vA);
#endif
            return;
        }
    }
...
void invalidateVRDueToConst(int reg, OpndSize size) {
    ...
    if(index >= 0 && compileTable[index].physicalReg != PhysicalReg_Null) {
        ...
        ALOGI("INVALIDATE virtual reg %d type %d\n", reg, LowOpndRegType_xmm);
#endif
        if(size == OpndSize_32)
...
#ifdef DEBUG_XFER_POINTS
                if(currentBB->xferPoints[k].dumpToXmm) ALOGI("XFER set_virtual_reg to xmm: xmm VR %d\n", reg);
#endif
                if(pType == LowOpndRegType_xmm)  {
#ifdef DEBUG_XFER_POINTS
                    ALOGI("XFER set_virtual_reg to memory: xmm VR %d\n", reg);
#endif
...
#ifdef DEBUG_XFER_POINTS
                ALOGI("XFER set_virtual_reg to memory: gp VR %d\n", reg);
#endif
            }
...
int registerAlloc(int type, int reg, bool isPhysical, bool updateRefCount) {
    ...
    ALOGI("%p: try to allocate register %d type %d isPhysical %d\n", currentBB, reg, type, isPhysical);
#endif
    if(currentBB == NULL) {
    ...
    if(tIndex < 0) {
      ALOGE("ERROR: reg %d type %d not found in registerAlloc\n", reg, newType);
      return PhysicalReg_Null;
    }
...
#ifdef DEBUG_REG_USED
        ALOGI("REGALLOC: allocate a reg %d\n", reg);
#endif
...
#ifdef DEBUG_REGALLOC
        ALOGI("already allocated to physical register %d\n", compileTable[tIndex].physicalReg);
#endif
        if(updateRefCount)
            decreaseRefCount(tIndex);
...
#ifdef DEBUG_REG_USED
        ALOGI("REGALLOC: register %d is free\n", allRegs[index].physicalReg);
#endif
    } else {
...
int registerAllocMove(int reg, int type, bool isPhysical, int srcReg) {
    if(srcReg == PhysicalReg_EDI || srcReg == PhysicalReg_ESP || srcReg == PhysicalReg_EBP)
        ALOGE("ERROR: can't move from srcReg EDI or ESP or EBP\n");
#ifdef DEBUG_REGALLOC
    ALOGI("in registerAllocMove: reg %d type %d srcReg %d\n", reg, type, srcReg);
#endif
    int newType = convertType(type, reg, isPhysical);
    if(newType & LowOpndRegType_scratch) reg = reg - PhysicalReg_SCRATCH_1 + 1;
    int index = searchCompileTable(newType, reg);
    if(index < 0) {
        ALOGE("ERROR: reg %d type %d not found in registerAllocMove\n", reg, newType);
        return -1;
    }
...
int getFreeReg(int type, int reg, int indexToCompileTable) {
    ...
    if(isVirtualReg(type)) { //find a callee-saved register
        ...
        if(index < 0) {
            ALOGE("ERROR VR %d %d not found in infoBasicBlock of currentBB %d (num of VRs %d)\n",
    ...
    /* handle request from a temporary variable or a glue variable */
        ...
        if(vr_num >= 0) {
            ...
            if(index3 < 0) {
                ALOGE("ERROR2 in tracing linkage to VR %d\n", vr_num);
                dvmAbort();
            }
            ...
            if(compileTable[index3].physicalReg == PhysicalReg_Null) {
                ...
                if(index2 < 0) {
                    ALOGE("ERROR1 in tracing linkage to VR %d\n", vr_num);
                    dvmAbort();
                }
#ifdef DEBUG_REGALLOC
...
#ifdef DEBUG_REGALLOC
                    ALOGI("check register %d with count %d\n", regCandidateT,
...
PhysicalReg spillForLogicalReg(int type, int reg, int indexToCompileTable) {
    ...
    if(index < 0) {
        dumpCompileTable();
        ALOGE("ERROR: no register to spill for logical %d %d\n", reg, type);
        dvmAbort();
    }
...
int spillLogicalReg(int spill_index, bool updateTable) {
    if((compileTable[spill_index].physicalType & LowOpndRegType_hard) != 0) {
        ALOGE("ERROR: can't spill a hard-coded register\n");
        dvmAbort();
    }
    int physicalReg = compileTable[spill_index].physicalReg;
    if(!canSpillReg[physicalReg]) {
#ifdef PRINT_WARNING
        ALOGI("WARNING: can't spill register %d\n", physicalReg);
#endif
...
#ifdef DEBUG_REGALLOC
    ALOGI("REGALLOC: UNSPILL logical reg %d %d with refCount %d\n", compileTable[spill_index].regNum,
           compileTable[spill_index].physicalType, compileTable[spill_index].refCount);
#endif
    return PhysicalReg_Null;
}
...
int spillVirtualReg(int vrNum, LowOpndRegType type, bool updateTable) {
    ...
    if(index < 0) {
        ALOGE("ERROR: can't find VR %d %d in spillVirtualReg\n", vrNum, type);
        return -1;
    }
...
void updateCurrentBBWithConstraints(PhysicalReg reg) {
    if(reg > PhysicalReg_EBP) {
        ALOGE("register %d out of range in updateCurrentBBWithConstraints\n", reg);
    }
...
int findVirtualRegInTable(u2 vA, LowOpndRegType type, bool printError) {
    ...
    if(k < 0 && printError) {
        ALOGE("ERROR findVirtualRegInTable virtual register %d type %d\n", vA, type);
        dvmAbort();
    }
    return k;
}
...
int isVirtualRegConstant(int regNum, LowOpndRegType type, int* valuePtr, bool updateRefCount) {
    ...
    if((isConstL && size == OpndSize_32) || (isConstL && isConstH)) {
        if(updateRefCount) {
            ...
            if(indexOrig < 0) ALOGE("ERROR: can't find VR in isVirtualRegConstant num %d type %d\n", regNum, type);
            decreaseRefCount(indexOrig);
        }
#ifdef DEBUG_CONST
        ALOGI("VR %d %d is const case\n", regNum, type);
#endif
        return 3;
    }
...
void printDefUseTable() {
    ...
    DefUsePair* ptr = currentBB->defUseTable;
    while(ptr != NULL) {
        ALOGI("  def @ %x of VR %d %d has %d uses\n", ptr->def.offsetPC,
              ptr->def.regNum, ptr->def.physicalType,
              ptr->num_uses);
        DefOrUseLink* ptr2 = ptr->uses;
        while(ptr2 != NULL) {
            ALOGI("    use @ %x of VR %d %d accessType %d\n", ptr2->offsetPC,
                  ptr2->regNum,
                  ptr2->physicalType,
                  ptr2->accessType);
            ptr2 = ptr2->next;
...
int updateVRAtUse(int reg, LowOpndRegType pType, int regAll) {
    ...
    for(k = 0; k < currentBB->num_xfer_points; k++) {
        if(currentBB->xferPoints[k].offsetPC == offsetPC &&
           ...
           currentBB->xferPoints[k].physicalType == pType) {
#ifdef DEBUG_XFER_POINTS
            ALOGI("XFER from memory to xmm %d\n", reg);
#endif
        ...
        for(k = 0; k < num_regs_per_bytecode; k++) {
            if(infoByteCode[k].accessType == REGACCESS_D ||
               ...
               infoByteCode[k].accessType == REGACCESS_DU) { //search defUseTable
                ...
                if(indexT == NULL) {
                    ALOGE("ERROR: def at %x of VR %d %d not in table\n",
...
#ifdef DEBUG_DSE
                    ALOGI("DSE: num_uses is %d for def at %d for VR %d %d\n", indexT->num_uses,
                          offsetPC, infoByteCode[k].regNum, infoByteCode[k].physicalType);
#endif
                }
            }
...
#ifdef DEBUG_DSE
                    ALOGI("DSE: remove usage at offsetPC %d reached by def at %d\n", ptrUse->offsetPC,
                           ptr->def.offsetPC);
#endif
...
int searchMemTable(int regNum) {
    ...
    ALOGI("WARNING: in searchMemTable can't find VR %d num_memory_vr %d\n", regNum, num_memory_vr);
    return -1;
}
/////////////////////////////////////////////////////////////////////////
...
void setVRToMemory(int regNum, OpndSize size) {
    ...
    if(size == OpndSize_64) indexH = searchMemTable(regNum+1);
    if(indexL < 0) {
        ALOGE("ERROR: VR %d not in memVRTable\n", regNum);
        return;
    }
...
bool isVRNullCheck(int regNum, OpndSize size) {
    if(size != OpndSize_32) {
        ALOGE("ERROR: isVRNullCheck size should be 32\n");
        dvmAbort();
    }
    int indexL = searchMemTable(regNum);
    if(indexL < 0) {
...
bool isVRBoundCheck(int vr_array, int vr_index) {
    ...
    if(indexL < 0) {
        ALOGE("ERROR @ isVRBoundCheck: VR %d not in memVRTable\n", vr_array);
        return false;
    }
...
void setVRNullCheck(int regNum, OpndSize size) {
    if(size != OpndSize_32) {
        ALOGE("ERROR: setVRNullCheck size should be 32\n");
        dvmAbort();
    }
    int indexL = searchMemTable(regNum);
    if(indexL < 0) {
...
void setVRBoundCheck(int vr_array, int vr_index) {
    int indexL = searchMemTable(vr_array);
    if(indexL < 0) {
        ALOGE("ERROR @ setVRBoundCheck: VR %d not in memVRTable\n", vr_array);
        return;
    }
...
int requestVRFreeDelay(int regNum, u4 reason) {
    ...
    if(indexL >= 0) {
        ...
        ALOGE("ERROR @ requestVRFreeDelay: VR %d not in memVRTable\n", regNum);
    }
    return indexL;
}
...
int getSpillIndex(bool isGLUE, OpndSize size) {
    ...
    ALOGE("ERROR: can't find spill position in spillLogicalReg\n");
    return -1;
}
...
bool isVRLive(int vA) {
    ...
    if(index < 0) {
        ALOGE("ERROR in finding VR %d in memTable\n", vA);
        return false;
    }
...
#ifdef PRINT_WARNING
    ALOGI("WARNING: offsetPC %d not in live range of VR %d\n", offsetPC, compileTable[k].regNum+1);
#endif
    return false;
}
...
#ifdef PRINT_WARNING
    if(!found) ALOGI("WARNING: offsetPC %d not in live range of VR %d\n", offsetPC, compileTable[k].regNum+1);
#endif
        ...
        if(compileTable[k].refCount == 0 && compileTable[k].physicalReg != PhysicalReg_Null) {
            ...
            if(typeA) {
                if(compileTable[k].spill_loc_index >= 0) {
                    ...
                    compileTable[k].spill_loc_index = -1;
                    ALOGE("ERROR: free a temporary register with TRSTATE_SPILLED\n");
                }
            }
        }
    }
...
int updateRefCount(int reg, LowOpndRegType type) {
    ...
    if(index < 0) {
        ALOGE("ERROR: virtual reg %d type %d not found in updateRefCount\n", reg, type);
        return -1;
    }
    decreaseRefCount(index);
    return 0;
...
int updateRefCount2(int reg, int type, bool isPhysical) {
    ...
    int index = searchCompileTable(newType, reg);
    if(index < 0) {
        ALOGE("ERROR: reg %d type %d not found in updateRefCount\n", reg, newType);
        return -1;
    }
    decreaseRefCount(index);
    return 0;
...
bool isGlueHandled(int glue_reg) {
    ...
    if(index < 0) {
        ALOGE("ERROR: glue reg %d not found in isGlueHandled\n", glue_reg);
        return -1;
    }
    if(compileTable[index].spill_loc_index >= 0 ||
       compileTable[index].physicalReg != PhysicalReg_Null) {
#ifdef DEBUG_GLUE
        ALOGI("GLUE isGlueHandled for %d returns true\n", glue_reg);
#endif
        return true;
    }
#ifdef DEBUG_GLUE
    ALOGI("GLUE isGlueHandled for %d returns false\n", glue_reg);
#endif
    return false;
}
...
void resetGlue(int glue_reg) {
    ...
    int index = searchCompileTable(LowOpndRegType_gp, glue_reg);
    if(index < 0) {
        ALOGE("ERROR: glue reg %d not found in resetGlue\n", glue_reg);
        return;
    }
#ifdef DEBUG_GLUE
    ALOGI("GLUE reset for %d\n", glue_reg);
#endif
    compileTable[index].physicalReg = PhysicalReg_Null;
    if(compileTable[index].spill_loc_index >= 0)
...
void updateGlue(int reg, bool isPhysical, int glue_reg) {
    ...
    int index = searchCompileTable(LowOpndRegType_gp, glue_reg);
    if(index < 0) {
        ALOGE("ERROR: glue reg %d not found in updateGlue\n", glue_reg);
        return;
    }
    ...
    if(index2 < 0 || compileTable[index2].physicalReg == PhysicalReg_Null) {
        ALOGE("ERROR: updateGlue reg %d type %d\n", reg, newType);
        return;
    }
#ifdef DEBUG_GLUE
    ALOGI("physical register for GLUE %d set to %d\n", glue_reg, compileTable[index2].physicalReg);
#endif
...
int checkVirtualReg(int reg, LowOpndRegType type, int updateRefCount) {
    ...
    int index = searchCompileTable(LowOpndRegType_virtual | type, reg);
    if(index < 0) {
        ALOGE("ERROR: virtual reg %d type %d not found in checkVirtualReg\n", reg, type);
        return PhysicalReg_Null;
    }
...
bool checkTempReg2(int reg, int type, bool isPhysical, int physicalRegForVR) {
    ...
    ALOGE("ERROR in checkTempReg2 %d %d\n", reg, newType);
    return false;
}
//!check whether a temporary can share the same physical register with a VR
...
int checkTempReg(int reg, int type, bool isPhysical, int vrNum) {
    ...
    int index = searchCompileTable(newType, reg);
    if(index < 0) {
        ALOGE("ERROR: temp reg %d type %d not found in checkTempReg\n", reg, newType);
        return PhysicalReg_Null;
    }
...
#ifdef DEBUG_REGALLOC
        ALOGI("WARNING: in checkTempReg, the temporary register %d %d was spilled\n", reg, type);
#endif
...
void globalVREndOfBB(const Method* method) {
    ...
    for(k = 0; k < num_compile_entries; k++) {
        ...
        if(isVirtualReg(compileTable[k].physicalType) &&
           ...
           compileTable[k].physicalReg != compileTable[k].physicalReg_prev) {
#ifdef DEBUG_ENDOFBB
            ALOGI("WARNING: end of BB GG VR is not mapped to the specific reg: %d %d %d\n",
                  compileTable[k].regNum, compileTable[k].physicalType, compileTable[k].physicalReg);
            ALOGI("ENDOFBB SPILL VR %d %d\n", compileTable[k].regNum, compileTable[k].physicalType);
#endif
...
#ifdef DEBUG_REFCOUNT
    ALOGI("REFCOUNT: to %d in nextVersionOfHardReg %d\n", refCount, pReg);
#endif
...
void insertFromVirtualInfo(BasicBlock_O1* bb, int k) {
    ...
    if(index < 0) {
        ...
        if(num_compile_entries >= COMPILE_TABLE_SIZE) {
            ALOGE("ERROR compileTable overflow");
            dvmAbort();
        }
    }
...
BasicBlock_O1* createBasicBlock(int src_pc, int end_pc) {
    ...
    if(num_bbs_for_method >= MAX_NUM_BBS_PER_METHOD) {
        ALOGE("ERROR number of basic blocks\n");
        dvmAbort();
    }
    return bb;
}
...
void rememberState(int stateNum) {
    ...
    for(k = 0; k < num_compile_entries; k++) {
        ...
        else ALOGE("ERROR: state table overflow!!!\n");
#ifdef DEBUG_STATE
...
void insertSrcReg(int srcPhysical, int targetReg, int targetSpill, int index) {
    ...
    for(k = 0; k < num_src_regs; k++) {
        if(srcRegs[k].physicalReg == srcPhysical) { //increase num_dests
            if(srcRegs[k].num_dests >= MAX_NUM_DEST) {
                ALOGE("ERROR exceed number dst regs for a source reg\n");
                dvmAbort();
            }
    ...
    if(num_src_regs >= MAX_NUM_DEST) {
        ALOGE("ERROR exceed number of source regs\n");
        dvmAbort();
    }
...
#ifdef DEBUG_STATE
    ALOGI("resetStateOfGlue state %d regNum %d\n", stateNum, compileTable[k].regNum);
#endif
    if(stateNum == 1) {
...
void constructSrcRegs(int stateNum) {
...
#ifdef DEBUG_STATE
                ALOGI("insert for VR Null %d %d %d\n", targetReg, targetSpill, k);
#endif
            }
...
#ifdef DEBUG_STATE
                    ALOGI("WARNING: in transferToState spill_loc_index is negative for temporary %d\n", compileTable[k].regNum);
#endif
                } else {
                    /* insert a xfer order from memory to targetReg */
#ifdef DEBUG_STATE
                    ALOGI("insert Null %d %d %d\n", targetReg, targetSpill, k);
#endif
                    insertSrcReg(PhysicalReg_Null, targetReg, targetSpill, k);
                }
            }
    ...
    while(num_handled < num_src_regs) {
        ...
        if(num_handled == prev_handled) {
            ALOGE("ERROR: no progress in selecting order\n");
            dvmAbort();
        }
    } //while
    for(k = 0; k < num_src_regs; k++) {
    ...
    if(num_in_order != num_src_regs) {
        ALOGE("ERROR constructing order\n");
        dvmAbort();
    }
#ifdef DEBUG_STATE
    ALOGI("ORDER: ");
...
void transferToState(int stateNum) {
    ...
    int k;
#ifdef DEBUG_STATE
    ALOGI("STATE: transfer to state %d\n", stateNum);
#endif
    if(stateNum > 4 || stateNum < 1) ALOGE("ERROR: state table overflow!!!\n");
    ...
    for(k = 0; k < num_memory_vr; k++) {
        ...
        if(targetReg != memVRTable[k].regNum)
            ALOGE("ERROR: regNum mismatch in transferToState\n");
        if(targetBool && (!memVRTable[k].inMemory)) {
            //dump to memory, check entries in compileTable: vA gp vA xmm vA ss
#ifdef DEBUG_STATE
            ALOGI("WARNING: inMemory mismatch for VR %d in transferToState\n", targetReg);
#endif
            bool doneXfer = false;
            ...
            if(!doneXfer) ALOGI("WARNING: can't match inMemory of VR %d in transferToState\n", targetReg);
        }
        if((!targetBool) && memVRTable[k].inMemory) {
            //do nothing
        }
