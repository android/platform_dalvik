    ...
    for(i = 0; i < 256; i++) {
        ...
        *tmp = i;
        ...
        if(stream - tmpStart  > 0)
            ALOGI("LOWER bytecode %x size in u2: %d ncg size in byte: %d\n", i, size_in_u2, stream - tmpStart);
    }
    exit(0);
}
...
int lowerByteCode(const Method* method) { //inputs: rPC & inst & stream & streamMethodStart
    ...
    if(offsetPC >= BYTECODE_SIZE_PER_METHOD) ALOGI("ERROR ERROR offsetPC %d exceeds BYTECODE_SIZE_PER_METHOD\n", offsetPC);
    ...
    case OP_BREAKPOINT:
        ...
        dvmAbort();
#if FIXME                       // not yet supported
    case OP_INVOKE_DIRECT_EMPTY:
        return op_invoke_direct_empty();
#endif
    case OP_IGET_QUICK:
