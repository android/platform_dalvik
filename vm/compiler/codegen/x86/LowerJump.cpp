...
int updateJumpInst(char* jumpInst, OpndSize immSize, int relativeNCG) {
    ...
    ALOGI("update jump inst @ %p with %d\n", jumpInst, relativeNCG);
#endif
    if(immSize == OpndSize_8) { //-128 to 127
        if(relativeNCG >= 128 || relativeNCG < -128) {
            ALOGE("ERROR: pre-allocated space for a forward jump is not big enough\n");
            dvmAbort();
        }
    }
...
int insertLabel(const char* label, bool checkDup) {
    ...
    globalShortMap = item;
#ifdef DEBUG_NCG
    ALOGI("insert short-term label %s %p\n", label, stream);
#endif
...
int insertLabelWorklist(const char* label, OpndSize immSize) {
    ...
    globalWorklist = item;
#ifdef DEBUG_NCG
    ALOGI("insert globalWorklist: %s %p\n", label, stream);
#endif
    return 0;
}
...
#ifdef DEBUG_NCG
    ALOGI("insert globalShortWorklist: %s %p\n", label, stream);
#endif
    return 0;
}
/*!
...
#ifdef DEBUG_NCG
    ALOGI("insertChainingWorklist: %p basic block %d\n", codeStart, bbId);
#endif
    return 0;
}
...
#ifdef DEBUG_NCG
    ALOGI("insert globalDataWorklist: %s %p \n", label, offset);
#endif
...
#ifdef DEBUG_NCG
    ALOGI("insert VMAPIWorklist: %s %p \n", label, offset);
#endif
    return 0;
}
////////////////////////////////////////////////
...
void performLabelWorklist() {
    ...
    while(ptr != NULL) {
#ifdef DEBUG_NCG
        ALOGI("perform work global %p for label %s\n", ptr->codePtr, ptr->label);
#endif
...
#ifdef DEBUG_NCG
    ALOGI("perform work ImmRM inst @ %p for label %s with %d\n", moveInst, label, relativeNCG);
#endif
...
#ifdef DEBUG_NCG_JUMP
            ALOGI("insert to short worklist %s %d\n", target, *immSize);
#endif
...
#ifdef DEBUG_NCG
    ALOGI("backward branch @ %p for label %s\n", stream, target);
#endif
...
#ifdef DEBUG_NCG
    ALOGI("insert NCGWorklist (goto forward) @ %p offsetPC %x relativePC %x offsetNCG %x\n", stream, offsetPC, relativePC, offsetNCG2);
#endif
...
#ifdef DEBUG_NCG_CODE_SIZE
                ALOGI("copy data section to stream %p: start at %d, %d bytes\n", stream, tmpPC, sz);
#endif
#ifdef DEBUG_NCG
                ALOGI("update data section at %p with %d\n", ptr->codePtr, stream-ptr->codePtr);
#endif
...
int getRelativeNCG(s4 tmp, JmpCall_type type, bool* unknown, OpndSize* size) {//tmp: relativePC
    ...
    *unknown = false;
    ...
    int offsetNCG2 = stream - streamMethodStart;
#ifdef DEBUG_NCG
    ALOGI("goto backward @ %p offsetPC %d relativePC %d offsetNCG %d relativeNCG %d\n", stream, offsetPC, tmp, offsetNCG2, tmpNCG-offsetNCG2);
#endif
    int relativeOff = tmpNCG - offsetNCG2;
