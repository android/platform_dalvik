...
bool dvmClassStartup(void)
{
     ...
     * ClassObject, so it's not fatal to not malloc.
     */
    gDvm.initiatingLoaderList = malloc(sizeof(InitiatingLoaderList)*
                                       ZYGOTE_CLASS_CUTOFF);
    if (gDvm.initiatingLoaderList != NULL) {
      memset(gDvm.initiatingLoaderList, 0,
             sizeof(InitiatingLoaderList)*ZYGOTE_CLASS_CUTOFF);
    }
