...
static pid_t forkAndSpecializeCommon(const u4* args, bool isSystemServer)
{
	...
	StringObject* StringObj = (StringObject*)args[5];
	if (StringObj)
            seinfo = dvmCreateCstrFromString(StringObj);
	StringObject* StringObj2 = (StringObject*)args[6];
	if (StringObj2)
            niceName = dvmCreateCstrFromString(StringObj2);
    }
